...
import { CateProps } from '@/helpers/definition';
import Image from 'next/image';


  const ButtonCategorias = ({FilCateProducts}:CateProps) => {
  return (
    <section className='w-full h-40 flex justify-around items-center p-15'>
        {categorias.map((categoria) => (
            <div key={categoria.id} className='flex flex-col items-center' onClick={()=>FilCateProducts(categoria.name)} >
